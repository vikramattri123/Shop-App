...
const Menshirt = () => {
    ...
    const arrow  = false;
     dispatch(StoreAction.changestatus({status,arrow}));
      const Fetch = async()=> 
      {
          ...
          let data= await collect.MenShirt;
          console.log(data);
  ...
  const result  =  item.filter((val)=> val.type == 'MenShirt');
  const size  = result.length;
console.log(result);
return (
  <>
  <div style={{width:'auto',height:'400px fixed',marginTop:'36px',marginTop:'65px'}} >
      <img src="https://shop.polymer-project.org/esm-bundled/images/mens_tshirts.jpg" class="img-fluid" alt="Responsive image"/>
     </div>
     <p style={{textAlign:'center',fontSize:'17px',fontWeight:'bold',fontFamily:'sans-serif',margin:'22px 27px 8px -20px',padding:'0px 5px'}}>Men's T-Shirt</p>
