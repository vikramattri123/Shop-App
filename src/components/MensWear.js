...
const MensWear = () => 
{
    ...
    useEffect(()=>
    {
        ...
        const Fetch = async()=> 
        {
            ...
            let data= await collect.MensWear;
            console.log(data);
    ...
    const result  =  item.filter((val)=> val.type == 'MenWear');
    const size  = result.length;
  console.log(result);
  return (
    <>
    <div style={{width:'auto',height:'400px fixed',marginTop:'36px',marginTop:'65px'}} className={classes.scrap}>
        <img src="https://shop.polymer-project.org/esm-bundled/images/mens_outerwear.jpg" class="img-fluid" alt="Responsive image"/>
       </div>
       <p style={{textAlign:'center',fontSize:'17px',fontWeight:'bold',fontFamily:'sans-serif',margin:'22px 27px 0px -12px',padding:'0px 5px 0px -22px'}}>Men's Outerwear</p>
       <p style={{textAlign:'center',fontSize:'12px',fontFamily:'sans-serif',margin:'9px 10px 0px -30px',padding:'0px 5px',color:'#757575'}}>({size} items)</p>
